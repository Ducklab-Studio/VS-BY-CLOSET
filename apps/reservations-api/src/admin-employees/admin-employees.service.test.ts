...
describe('AdminEmployeesService — CRUD de funcionários (integração real, Neon)', () => {
  ...
  test('5) remover funcionário revoga sessão, impede login, e não pode mais ser reativado', async () => {
    const owner = await createSuperAdmin();
    const phone = freshPhone();
  ...
  test('8) SUPER_ADMIN nunca pode ser alvo de block/reactivate/remove/updatePermissions por este service', async () => {
    const owner = await createSuperAdmin();
