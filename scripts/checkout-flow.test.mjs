...
const activeFiles = (path) =>
  readdirSync(new URL(path, root), { recursive: true, withFileTypes: true })
...
const input = {
  items: [{ shopifyVariantId: '123', quantity: 1 }],
  pickupDate: '2030-10-15',
  termsAccepted: true,
};
...
function fixture(overrides = {}) {
  const calls = [];
  const deps = {
    async createHold(value) {
      calls.push(['hold', value]);
      return held;
    },
    ...
    storeLastReservation(...args) {
      calls.push(['store', ...args]);
    },
    ...overrides,
  };
  const { createCheckoutAttempt } = load('apps/marketing/src/lib/checkout-attempt.ts', {
    require: () => deps,
  });
  return {
    flow: createCheckoutAttempt(),
    calls,
    before: async () => {
      calls.push(['stock']);
    },
...
test('Sunday choice must be resolved before a HOLD can open checkout', async () => {
  ...
  const { flow, calls, before } = fixture({
    async createHold(value) {
      keys.push(value.idempotencyKey);
      return value.sundayReturnOption
        ? held
        : { ok: false, reason: 'needs_sunday_choice', returnOptions: [] };
    },
  });
...
test('quantity, pickup or Sunday option changes cannot reuse the previous HOLD', async () => {
  ...
  assert.equal(
    new Set(calls.filter(([kind]) => kind === 'hold').map(([, value]) => value.idempotencyKey))
      .size,
    4,
  );
});
...
test('only authoritative expiration releases the cached HOLD for a new attempt', async () => {
  const { flow, calls, before } = fixture({
    async createCheckout() {
      return { ok: false, expired: true, message: 'expired' };
    },
  });
  await flow.run(input, before);
  await flow.run(input, before);
...
test('legacy calendar cannot fetch, invent availability or mutate Shopify carts', () => {
  ...
  runInNewContext(read('apps/shopify-app/extensions/rental-calendar/assets/rental-calendar.js'), {
    document,
    fetch: () => {
      assert.fail('Legacy calendar must not access the network');
    },
  });
...
test('Railway uses the package start command and preserves migrations and health', () => {
  ...
  assert.deepEqual(railway.deploy.preDeployCommand, [
    'pnpm --filter @valle/reservations-api run db:migrate',
  ]);
});
...
test('no draft config can target the production app, and production scopes are never placeholders', () => {
  ...
  const clientIdOf = (file) =>
    (read(`apps/shopify-app/${file}`).match(/^client_id\s*=\s*"([^"]*)"/m) ?? [])[1];
  const scopesOf = (file) =>
    (read(`apps/shopify-app/${file}`).match(/^scopes\s*=\s*"([^"]*)"/m) ?? [])[1];
  ...
  assert.doesNotMatch(
    scopesOf('shopify.app.production.toml') ?? '',
    /PLACEHOLDER/,
    'a config de produção nunca pode ir ao ar com scopes placeholder',
  );

  const drafts = readdirSync(new URL('apps/shopify-app/', root)).filter(
    (file) => /^shopify\.app\..*\.toml$/.test(file) && file !== 'shopify.app.production.toml',
  );
  assert.ok(
    drafts.length > 0,
    'a guarda só faz sentido se houver outros arquivos de config para vigiar',
  );
...
test('active application code has no direct Mercado Pago integration', () => {
  ...
  assert.doesNotMatch(
    source,
    /mercado\s*pago|mercadopago|preference_id|notification_url|mp_access_token|mp_public_key/i,
  );
});
...
test('product description keeps its formatting but never executable content', () => {
  ...
  for (const payload of [
    ...
    assert.doesNotMatch(
      clean,
      /<\s*(script|iframe|object|embed|style|form|base|svg|animate|xmp)\b/i,
      payload,
    );
...
test('the storefront ships a CSP and a Permissions-Policy', () => {
  ...
  assert.doesNotMatch(
    config,
    /connect-src[^`]*https:\/\/[a-z0-9-]+\.(up\.railway\.app|vercel\.app)/i,
  );
});
...
function loadValePassProduct(env = {}) {
  return load('apps/marketing/src/lib/vale-pass-product.ts', {
    process: {
      env: {
        NEXT_PUBLIC_VALE_PASS_PRODUCT_ID: undefined,
        NEXT_PUBLIC_VALE_PASS_VARIANT_ID: undefined,
        ...env,
      },
    },
...
test('1) Valle Pass é identificado tanto por ID numérico quanto por GID da Shopify', () => {
  ...
  assert.equal(
    isValePassProduct({ variantId: `gid://shopify/ProductVariant/${REAL_VARIANT_ID}` }),
    true,
  );
});
...
test('1) a página da peça só renderiza ValePassPresentation (nunca RentalCalendar) quando o produto é o Valle Pass', () => {
  ...
  assert.match(
    page,
    /isValePassProduct\(\{\s*productId:\s*product\.id,\s*variantId:\s*variant\?\.id\s*\}\)/,
  );
...
test('2) a apresentação do Valle Pass nunca chama HOLD, disponibilidade ou reserva', () => {
  ...
  assert.doesNotMatch(
    source,
    /createHold|fetchAvailability|addRentalToCart|RentalCalendar|NEXT_PUBLIC_HOLDS_URL|NEXT_PUBLIC_AVAILABILITY_URL|'use client'/,
  );
...
test('4) o checkout do Valle Pass continua funcionando — link direto pro carrinho oficial da Shopify', () => {
  ...
  assert.equal(
    valePassCheckoutHref(REAL_VARIANT_ID),
    `https://loja-teste.myshopify.com/cart/${REAL_VARIANT_ID}:1`,
  );
  assert.equal(
...
test('Valle Pass: IDs de configuração por env (numérico ou GID) sobrepõem o default', () => {
  const { isValePassProduct } = loadValePassProduct({
    NEXT_PUBLIC_VALE_PASS_PRODUCT_ID: 'gid://shopify/Product/9999999999',
  });
